...
初期化時に5シートを作成し、1行目に以下のヘッダーを**厳密に**設定すること。カラムの追加・削除・順序変更・名前変更は**ユーザーの許可をとること**。
...
| K | `状態メモ` | String | |
| L | `単位換算JSON` | String | `{"fromQty":1,"fromUnit":"パック","toQty":150,"toUnit":"g"}` |
...
| F | `紐づくレシピ名` | String | |
| G | `出自種別` | String | 手動追加 / レシピ詳細 / 献立スケジュール |
| H | `予定日` | Date/String | 献立スケジュール由来のみ |
| I | `食事区分` | String | 朝 / 昼 / 夜 |
