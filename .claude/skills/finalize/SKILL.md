...
## 手順
1. TKT を取得する。該当 eval と未閉 gate が不明な場合のみ `/check-gates <TKT>` を実行する
   （このセッションで直近に実行済みなら**スキップ**し、最後の手順5の1回に集約する）。
2. **`verify.json` が今回のコード変更後に生成済みなら `/verify` を再実行しない**。
   未実行・古い場合のみ `/verify <TKT>` を実行する。出力は結果（pass/fail と policy 判定）だけ要約し、
   build ログ全文を会話に展開しない。
...
6. **ナレッジ層を更新する**（各ファイルは追記対象セクションの周辺だけ読めばよい。全文 Read 不要）:
