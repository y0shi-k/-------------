...
- 食材管理は `inventory-board.tsx`。買い物リスト表示は `shopping-list-section` を使っている。
- 現コードでは `today-dashboard` は `InventoryBoard` に内包されていない。TKT-0158では触らず、ホーム集約は TKT-0162 で扱う。
- TKT-0158では `shopping-list-section` の挙動変更は行わない。
