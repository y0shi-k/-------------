...
0p. (消費フローの単位不一致救済・2026-06-13 /plan-initiative→/breakdown 展開) レシピと在庫の単位が食い違うケース（じゃがいも100g vs 在庫3個）の料理完了消費を救済。①TKT-0248=unit_conversion の逆方向換算対応（土台・純粋関数のみ）②TKT-0249=消費ダイアログで在庫選択時に入力単位を在庫単位へ切替（換算未登録でも在庫単位で入力・減算。その場での換算登録UIはユーザー判断で見送り）。依存順 0248→0249。両方🟢非危険（schema/auth/RLS無変更・`pwa_mobile_ui`）。
