...
0g. (レシピ並び替え拡張＋材料/調味料サブグルーピング・2026-06-07 /breakdown 展開／依存順) TKT-0197 の全画面ビュー並び替えを編集画面へ展開し、材料・調味料の中をサブグループ化（材料=A/B/C、調味料=あ/い/う 自動採番、行クリック＋Cmd/Ctrl複数選択→ラベル隣のグルーピング/解除ボタン）。グルーピングはDB永続保存。TKT-0198/0199/0200 完了（changelog 参照）。残2 TKT:
   - TKT-0201（🟢非危険・TKT-0200依存）: 全画面ビューのサブグルーピングUI（選択→グループ化/解除・自動ラベル・D&Dのグループ境界対応）。
   - TKT-0202（🟢非危険・TKT-0200/0201/0198依存）: 編集画面のサブグルーピングUI。0201の共通ロジックを流用、0198のD&Dと整合。
