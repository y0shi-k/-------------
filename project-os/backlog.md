...
0n. (タブレット不具合2点+辞書潜在バグ・2026-06-12 起票) **TKT-0238〜0240 全3チケット完了**（changelog 参照／保存場所ピッカーのlabelネスト解消・消費ダイアログの在庫リフェッチ・ユーザー辞書のグループ推移統合）。残るユーザー作業は実機スモークのみ（0238=タブレットで在庫編集→保存場所/単位をタップ選択→「その他」へ化けない、0239=再読込なしで食材管理の追加在庫が料理完了の自動マッチに出る。出なければ在庫側の単位/分類を確認）。cooking-record-edit-modal の在庫鮮度は意図的見送り（decisions 参照・必要なら別チケット）。
