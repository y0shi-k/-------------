...
0q. (材料/調味料D&Dの種別またぎ＋モバイルgrouping/タッチ並び替え・2026-06-13 /plan-initiative→/breakdown 展開・SPEC-0255) レシピ編集＆調理詳細の材料リストで①種別をまたいだ並び替え（種別は保持＝混在）②混在時に種別ラベルを隠すオプション（レシピ単位で永続化）③上部の材料/調味料フィルタタブは維持④モバイルのグルーピング複数選択（選択モード切替ボタン）⑤タッチ端末のD&D並び替え。依存順5チケット（採番済み・未着手）:
   - TKT-0255（🟢土台・先行）: 編集モーダルの材料エディタをフィルタタブ＋単一リスト化し、moveIngredient の種別またぎブロック撤廃（item_type 保持で並び替え）。
   - TKT-0256（🟢・0255依存）: 調理詳細ビューも同様に。moveCookingIngredient の item_type 書き換えを撤廃し単一混在リスト化。フィルタタブ維持。
   - TKT-0257（🔴schema・0255/0256依存）: 「種別ラベルを隠す」をレシピ単位で永続化（recipes に hide_ingredient_type_label bool 列追加＝is_favorite/TKT-0167 と同型）。manual-smokes/review 必須。db reset 禁止・migration 追加で適用。
   - TKT-0258（🟢・0255/0256依存）: グルーピング複数選択を「選択モード切替ボタン」でモバイル対応（長押しは不採用）。Cmd/Ctrl 併存・単一種別ガード維持。
   - TKT-0259（🟢・0255/0256/0258依存）: 材料リストのタッチ対応D&D（推奨 @dnd-kit を材料リスト限定導入／代替は自前pointerフック）。手順・ジャンルは対象外。
