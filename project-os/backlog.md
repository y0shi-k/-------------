...
0o. (在庫の全画面即時反映＝共有ストア化・2026-06-12 /plan-initiative→/breakdown 展開・SPEC-0242) 調理完了後に在庫一覧へ戻ってもリロードまで反映されない構造問題（3ボード常時マウント×useState複製×router.refresh依存）を、shell配下の共有在庫ストアContextへの持ち上げで解消（ユーザー承認済み・learnings 2026-06-12 の「必要になるまでやらない」を解除）。全🟢非危険（クライアントstate再構成のみ・schema/auth/RLS無変更）。依存順: TKT-0242（土台・**完了**）→TKT-0243（献立側移行・**完了＝発端不具合の解消点。残: 実機スモーク**、changelog 参照）→TKT-0244（残経路統一・**完了**、changelog 参照）→**TKT-0245（次の候補: クロスボード回帰テスト）**。0n の別チケット候補②（料理完了後の食材管理ボード即時反映）はこのイニシアチブに吸収。
