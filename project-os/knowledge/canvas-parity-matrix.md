...
| --- | --- | --- | --- | --- |
| 全体ナビ | 下部ナビで食材管理/献立・レシピ/料理・記録を切替 | Web版は単一ホーム上に各ワークスペースを縦配置。Canvas版の下部3モード切替とは違う | `changed` | TKT-0114候補: Canvas同等のモードナビ/スマホ導線 |
| 起動/ログイン | Canvas起動後にGAS初期化 | Web版はログイン後にSupabaseから本人データを初期表示 | `supabase_replace` | 置換として許容 |
| ステータス表示 | 常設ステータス、処理中表示、AIキャンセル、同期状態 | Web版は操作ごとのメッセージ中心。常設ステータスやAIキャンセルは不足 | `partial` | TKT-0114候補に含める |
| 食材一覧 | 保存場所タブ、使い切り、期限、ソート、選択操作 | 在庫一覧、編集、削除、期限表示はある。保存場所タブ、ソート、一括選択、使い切り導線は不足 | `partial` | TKT-0115候補: 在庫一覧のCanvas同等化 |
| 登録待ち | 画像スキャン、手動追加、AI解析、在庫確定、一括削除 | 手動追加、写真AI解析、在庫確定、個別削除はある。一括削除や登録ハブUIは不足 | `partial` | TKT-0115候補に含める |
| 写真AI食材解析 | 写真から候補を作り、登録待ちで確認して在庫化 | Web版は非公開Storage保存、サーバー側Gemini解析、登録待ち追加がある | `supabase_replace` | APIキー非露出方針で許容 |
| 保存場所管理 | 保存場所追加/削除/在庫あり制御 | 自由入力はあるが、保存場所マスタ管理と削除制御がない | `missing` | TKT-0116候補: 保存場所管理 |
| 単位換算 | 在庫に換算情報を持ち、消費時に使う | schema/UI/消費処理のいずれも未対応 | `missing` | TKT-0117候補: 単位換算schema/UI/消費連携 |
| 買い物リスト | 出自別表示、手動追加、購入済み、一括削除 | 献立不足から未購入リスト作成と表示はある。手動追加、購入済み、一括削除、出自別グループ操作は不足 | `partial` | TKT-0118候補: 買い物リスト完全化 |
| 不足食材選択 | タブ付き選択モーダルで買い物追加対象を選ぶ | 選択中献立の不足食材チェックはある。同じ単位だけ比較で、Canvas版の選択モーダルより簡易 | `partial` | TKT-0118候補に含める |
| レシピ集 | 一覧、詳細、作成、編集、削除、検索、ソート | 作成、編集、一覧、詳細はある。削除、検索、ソート、カード判断材料は不足 | `partial` | TKT-0119候補: レシピ集完全化 |
| レシピジャンル | AppSheet風複数選択、並び替え、省略表示 | カンマ区切り入力のみ。Canvas版の複数選択UIとは違う | `changed` | TKT-0119候補に含める |
| レシピ材料構造 | 食材/調味料/下準備/調理工程を分離 | Supabase上も `recipe_ingredients.item_type`, `prep_steps`, `steps` で分離済み | `same` | 現状維持 |
| レシピ編集 | 材料/調味料/工程の追加削除、ドラッグ並び替え | 行追加/削除はある。ドラッグ並び替えとCanvas同等の編集UIは不足 | `partial` | TKT-0119候補に含める |
| AIレシピ考案 | 期限切れ優先/指定食材/必須任意/AI相談/プレビュー保存 | Web版にはAIレシピ考案がない | `missing` | TKT-0120候補: AIレシピ考案APIとUI |
| レシピ本文AI構造化 | テキスト貼り付けからAIでレシピ化 | Web版には本文構造化がない | `missing` | TKT-0120候補に含める |
| 献立 | 7日表示、日送り、レシピ選択、ドラッグ移動、削除 | 日付指定で献立追加、一覧、完了はある。7日表示、日送り、ドラッグ移動、削除は不足 | `partial` | TKT-0121候補: 献立UI完全化 |
| 作りたい候補 | 候補キュー、理由チップ、スケジュール割当 | Web版には候補キューがない | `missing` | TKT-0122候補: 作りたい候補 |
| 今日ダッシュボード | 今日の献立/期限/買い物/同期/候補を統合表示 | Web版には統合ダッシュボードがない | `missing` | TKT-0123候補: 今日ダッシュボード |
| 調理ビューア | 材料/調味料タブ、在庫チェック、手順タブ、材料タップ照合 | Web版はレシピ詳細表示と献立完了のみ。調理中画面はない | `missing` | TKT-0124候補: 調理ビューア |
| 調理完了 | 消費量調整、代替品選択、在庫減算、履歴作成 | 献立ステータス完了と料理履歴作成のみ。在庫減算、消費量調整、代替品選択はない | `partial` | TKT-0125候補: 調理完了/在庫消費 |
| 料理記録 | 写真、評価、感想、履歴保存 | 手動料理履歴、写真圧縮、非公開Storage、評価、メモ保存はある | `supabase_replace` | 置換として許容。ただし調理完了連携はTKT-0125で補う |
| 料理履歴 | タイムライン/カレンダー/分析、検索、フィルタ、写真表示 | 履歴一覧と写真表示はある。カレンダー、分析、検索、フィルタは不足 | `partial` | TKT-0126候補: 料理履歴分析/検索 |
| 削除確認 | 独自確認モーダルで統一 | Web版は削除ボタン即実行の箇所がある。Canvas版の確認体験と違う | `changed` | TKT-0127候補: 削除確認統一 |
| スマホUI | Canvas版のスマホ前提UI、下部ナビ、安全なタップ領域 | Web版はレスポンシブ前提だが、Canvas同等の下部操作導線ではない | `partial` | TKT-0114/TKT-0128候補: スマホ/PWA仕上げ |
...
- `missing` と `changed` の後続チケット候補を正式チケット化する。
