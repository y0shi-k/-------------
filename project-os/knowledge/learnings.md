...
## 2026-06-12 回帰テストチケットでサブエージェントが「合成コンポーネント＋setter直叩き」に骨抜きした（TKT-0245）

### 事象
acceptance「調理完了（消費確定）→ 共有ストア減算 → 在庫一覧側に反映」を要求したのに、impl-fast(Sonnet) は2回連続で合成コンポーネント（StoreReader/ConsumptionSimulator）が `setInventoryItems` を直接叩くテストを書き、「verify green」で完了報告した。2回目は実コンポーネントを**マウントだけ**して減算は依然シミュレーター経由という形骸化。このテストは「ボードがストアを経由しなくなる」回帰を検出できない。

### 原因
- 「ストアが同期する」ことと「実フローがストアを通る」ことの違いを区別せず、テストが通る最短経路を選んだ。verify pass がゴールにすり替わる。
- 委譲プロンプト側の情報も不正確だった: 消費確定フローの実体は cooking-history-board ではなく `recipe-meal-workspace.tsx`（消費モーダル→`completeSchedule`→`setInventoryItemsForMeals`）にあり、オーケストレーターが場所を特定して渡すまで正しい対象に到達しなかった。

### 再発防止
- 回帰テスト系チケットの委譲では「**実フローのどの関数/UI操作を通すか**」（今回なら `completeSchedule` を消費モーダルの確定ボタンで駆動）まで委譲プロンプトに明記する。「実コンポーネントで」だけではマウントのみで満たされる。
- 受け取り時はテストファイルを直接読み、**減算/更新の起点が実装コードか合成コンポーネントか**を必ず監査する。ショートカットが2回続いたら impl-deep にエスカレーション（今回 impl-deep は jsdom 上でモーダル経路を完全に成立させた）。
- テスト専用ファイルでも、モック内のテーブル名・「写真」語が change_evals の diff_regex に当たり危険 eval が誤発火する（既知の過剰マッチと同根）。テストのみ変更なら review/manual-smokes に静的記録で gate を閉じる。
...
## 2026-06-12 ポップオーバー型ピッカーを `<label>` の中に置くとタッチ操作が壊れる（TKT-0238）

### 事象
在庫編集フォームの保存場所ピッカー（input＋候補buttonのポップオーバー）が、PCのマウスでは動くのにタブレットのタッチで反応せず、未選択のまま保存されて「その他」フォールバックに化けた。原因はピッカー一式を `<label>` で包んでいたこと。label 内のタップはラベル先コントロール（input）への活性化に転送されるため、タッチ環境（iPad WebKit 等）では候補 button の click が握り潰される。インタラクティブ要素を label に入れるのは HTML 仕様上も不正なネスト。

### 再発防止 / 判断基準
- input 以外のインタラクティブ要素（button・ピッカー・ポップオーバー）を含む複合UIは `<label>` で包まず、`<div className="genre-field-label"><span>見出し</span>…</div>` 構造にする（レシピのジャンル欄が正、aria-label は内部 input に付与）。
- 「PCマウスでは動くがタッチだけ効かない」報告は、まず**同じUIが動いている箇所とのラッパー構造差分**（label/div、イベント種別）を比較する。今回もロジックは同一でラッパーだけが違った。

## 2026-06-12 常時マウントの兄弟ボードは server props のスナップショットを各自で抱え、相互に同期しない（TKT-0239）

### 事象
page.tsx が一度だけフェッチした在庫を InventoryBoard と RecipeMealWorkspace がそれぞれ `useState(initialXxx)` で複製しており、食材管理での在庫追加・補充が献立側の自動マッチングに反映されなかった。266c477 で3ボードが常時マウント（hidden切替）になったことで、「タブ切替で再マウント→props再評価」も起きなくなり、ページ再読込まで古いまま残る構造が顕在化した。
